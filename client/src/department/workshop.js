...
function Workshop() {
  ...
  const getUser = async () => {
    try {
      const response = await fetch("/workshop", { method: "GET" });
      ...
      if (response.ok) {
        ...
        if (
          ...
          result.sessionData.department_name === "Workshop"
        ) {
          ...
          setError({
            ...
            message: "You don't have Workshop Privileges",
          });
        }
      } else {
        setError({
          status: response.status,
          message: "Access denied, you might not have Workshop Privileges",
        });
      }
    } catch (err) {
      setError({
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="text-center">
        <p className="text-gray-600 mb-4">
          ...
          | {error?.error || "Unauthorized access, you might not have Workshop Priviledges"}
        </p>
    ...
    <div className="flex">
      ...
      <div className="flex-1">
        ...
        <div className="w-full border border-none p-5 bg-gray-50 ml-64">
          <h1 className="text-4xl text-black mb-2 uppercase">
